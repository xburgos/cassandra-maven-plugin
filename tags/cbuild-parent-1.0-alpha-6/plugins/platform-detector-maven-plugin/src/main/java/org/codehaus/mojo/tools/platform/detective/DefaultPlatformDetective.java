...
public class DefaultPlatformDetective implements Contextualizable, LogEnabled, PlatformDetective
{
     ...
     */
    public String getArchitectureToken( MavenSession session ) throws PlatformDetectionException
    {
        scanArchitectureInfo( session );
     ...
     */
    public String getOperatingSystemToken( MavenSession session ) throws 
        PlatformDetectionException, ComponentLookupException
    {
        scanOperatingSystemInfo( session );

        PlatformPropertyPatterns platformPatterns = getPlatformPatterns( session );

        return platformPatterns.getOperatingSystemToken( releaseSource, rawOSInfo );
    }

    protected PlatformPropertyPatterns getPlatformPatterns( MavenSession session )
        throws ComponentLookupException
    {
        PlatformPropertyPatterns pp = (PlatformPropertyPatterns) container.lookup( 
            PlatformPropertyPatterns.ROLE, PlatformPropertyPatterns.ROLE_HINT );
        PlatformPropertyPatterns platformPatterns = pp.readFromContext( session );

        if ( platformPatterns == null )
        {
            getLogger().debug( "Can't get PlatformPropertyPatterns; using defaults..." );
    ...
    protected void scanOperatingSystemInfo( MavenSession session ) throws PlatformDetectionException
    {
        if ( rawOSInfo == null )
        {
            Map distroDetectors;
            ...
            for ( Iterator it = distroDetectors.entrySet().iterator(); it.hasNext(); )
            {
                ...
                getLogger().debug( "Detector: " + detector.getClass().getName() + " enabled: " + detector.isEnabled() );
                ...
                if ( detector.isEnabled() )
                {
                    ...
                    getLogger().debug( "releaseSource = " + releaseSource );
    ...
    protected void scanArchitectureInfo( MavenSession session ) throws PlatformDetectionException
    {
