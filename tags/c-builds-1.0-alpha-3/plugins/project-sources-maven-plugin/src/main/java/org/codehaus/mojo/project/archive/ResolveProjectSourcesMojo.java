    ...
    extends AbstractProjectSourcesMojo
{
    ...
    /**
     * Flag denoting whether the current project build directory can be used as a valid sourceLocation. Projects will
     * either have a source archive from elsewhwere (third-party build), or it will have an initial sourceLocation of
     * the current project directory (internal projects).
     * 
        ...
        throws MojoExecutionException, MojoFailureException
    {
        ...
        {
            getLog().debug( "Something is wrong. Does sourceArchiveFile \'" + sourceArchiveFile + "\' exist? " + sourceArchiveFile.exists() );
        ...
        if ( sourceArchiveFile.isDirectory() )
        {
            if ( !projectDirIsSourceLocation )
            {
                throw new MojoExecutionException( "Project sources cannot originate from a directory. "
                                + "Please specify a sourceLocation that is an archive file." );
            }
            else
            {
