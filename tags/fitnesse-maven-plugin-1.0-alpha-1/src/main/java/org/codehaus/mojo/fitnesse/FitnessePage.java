...
public class FitnessePage
{
    ...
    public String getStatus() throws MojoExecutionException
    {
        try
        {
            ...
            if ( mResultContent.contains( "document.getElementById(\"test-summary\").className = \"pass" ) )
            {
                return STATUS_OK;
            }
            else if ( mResultContent.contains( "document.getElementById(\"test-summary\").className = \"fail" ) )
            {
                return STATUS_FAIL;
            }
            else if ( mResultContent.contains( "document.getElementById(\"test-summary\").className = \"error" ) )
            {
                return STATUS_ERROR;
            }
            else
