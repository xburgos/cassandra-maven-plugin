...
public class StartAndWaitMojo extends StartMojo {
  ...
  public void execute() throws MojoExecutionException {
    ...
    try {
      ...
      throw new MojoExecutionException("Unable to instantiate naming context: "
          + e.getMessage(), e);
    }
      ...
      try {
        ...
        if (i > retry) {
          throw new MojoExecutionException(
              "Unable to get JBoss jmx MBean connection: " + e.getMessage(), e);
        }
    ...
    while (!isStarted && System.currentTimeMillis() - startTime < timeout) {
      try {
        ...
        isStarted = ((Boolean) s.getAttribute(serverMBeanName, "Started")).booleanValue();
      } catch (Exception e) {
