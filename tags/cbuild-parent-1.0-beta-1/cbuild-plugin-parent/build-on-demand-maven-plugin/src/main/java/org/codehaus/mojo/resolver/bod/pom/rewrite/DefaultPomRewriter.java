    ...
    implements PomRewriter, Contextualizable
{
        ...
        if ( projectBuilder instanceof DefaultMavenProjectBuilder )
        {
            try
            {
                ...
                for ( int i = 0; i < fields.length; i++ )
                {
                    ...
                    try
                    {
                        ...
                        Map < String, MavenProject > cache = (Map) fields[i].get( projectBuilder );
            ...
            catch ( IllegalAccessException e )
            {
                throw new ProjectBuildingException( project.getId(),
                    "Failed to purge project-builder cache, IllegalAccessException" );
            }
            catch ( SecurityException e )
            {
                throw new ProjectBuildingException( project.getId(),
                    "Failed to purge project-builder cache, SecurityException" );
            }
        }
    }

    public List < MavenProject > rewrite( List < MavenProject > projectInstances, PomRewriteConfiguration configuration,
                         ArtifactRepository localRepository, MessageHolder errors )
    {
        ...
        if ( configuration == null || configuration.isEmpty() )
        {
            ...
            for ( Iterator < MavenProject > it = projectInstances.iterator(); it.hasNext(); )
            {
        ...
        List < MavenProject > changedInstances = new ArrayList < MavenProject > ( projectInstances.size() );
...
//        if ( deps != null )
//        {
//            for ( Iterator dependencyIterator = deps.iterator(); dependencyIterator.hasNext(); )
//            {
...
//        }
        ...
        throws IOException, ProjectBuildingException
    {
        ...
        //DefaultProfileActivationContext defActCtxt = new DefaultProfileActivationContext(defProps, Boolean.FALSE);
        //ProfileManager profileMgr = new DefaultProfileManager( container, defActCtxt );
