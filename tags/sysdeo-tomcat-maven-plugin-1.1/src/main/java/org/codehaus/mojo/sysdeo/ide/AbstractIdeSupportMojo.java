    ...
    implements LogEnabled
{
        ...
        throws MojoExecutionException, MojoFailureException
    {
        ...
        {
            getLog().warn(
                "downloadSources is deprecated. Configure the eclipse plugin to download sources/javadoc bundles" );
        }
        ...
        if ( dependencies != null )
        {
            ...
            for ( Iterator i = artifactResolutionResult.getArtifactResolutionNodes().iterator(); i.hasNext(); )
            {
                ...
                if ( !isReactorProject || emittedReactorProjectId.add( art.getGroupId() + '-' + art.getArtifactId() ) )
                {
                    ...
                    IdeDependency dep =
                        new IdeDependency( art.getGroupId(), art.getArtifactId(), art.getVersion(), isReactorProject,
                            Artifact.SCOPE_TEST.equals( art.getScope() ),
                            Artifact.SCOPE_SYSTEM.equals( art.getScope() ), Artifact.SCOPE_PROVIDED.equals( art
                                .getScope() ), art.getArtifactHandler().isAddedToClasspath(), art.getFile(), art
                                .getType(), null, null );

                    dependencyList.add( dep );
                    if ( workspaceProjects.contains( dep ) )
                    {
                        IdeDependency workspaceProject =
                            (IdeDependency) workspaceProjects.get( workspaceProjects.indexOf( dep ) );
