...
public class DashBoardReportMojo extends AbstractMavenReport
{
    ...
    /**
     * Used to generate a dashboard report as Maven 1, only for multi-modules project.
     * it also merges the 2 reports (Summary and detailled) as only one
     * @parameter expression="${m1LikeRendering}"
     *            default-value="false"
     */
    ...
    protected void executeReport( Locale arg0 ) throws MavenReportException
    {
        ...
        if ( canGenerate )
        {
            ...
            if ( mavenProject != null )
            {
                ...
                if ( project.getModules() != null && project.getModules().size() > 0 && m1LikeRendering )
                {
                    reportGenerator = new DashBoardMaven1ReportGenerator( mavenProject, isPropHibernateSet, getLog() );
                }
                else
                {
                    reportGenerator = new DashBoardReportGenerator( mavenProject, isSummary, isPropHibernateSet, getLog() );
                }
