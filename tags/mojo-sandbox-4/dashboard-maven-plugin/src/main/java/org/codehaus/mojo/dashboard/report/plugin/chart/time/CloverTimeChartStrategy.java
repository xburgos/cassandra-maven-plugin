...
public class CloverTimeChartStrategy extends AbstractTimeChartStrategy
{
    ...
    public void fillDataset()
    {
        ...
        while ( iter.hasNext() )
        {
            ...
            s1.addOrUpdate( getChartDate( this.timePeriod.normalize( date ) ),  ( covered / (double) total ) * 100 );
            
            int totalCond = clover.getConditionals();
            int coveredCond = clover.getCoveredConditionals();
            s2.addOrUpdate( getChartDate( this.timePeriod.normalize( date ) ), ( coveredCond / (double) totalCond ) * 100 );
            
            int totalStat = clover.getStatements();
            int coveredStat = clover.getCoveredStatements();
            s3.addOrUpdate( getChartDate( this.timePeriod.normalize( date ) ), ( coveredStat / (double) totalStat ) * 100 );
            
            int totalMeth = clover.getMethods();
            int coveredMeth = clover.getCoveredMethods();
            s4.addOrUpdate( getChartDate( this.timePeriod.normalize( date ) ), ( coveredMeth / (double) totalMeth ) * 100 );
