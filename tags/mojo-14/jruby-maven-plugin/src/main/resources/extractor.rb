...
GOAL = 'goal';
CLASS_ARGS = [ 'configurator', 'phase', 'requiresDependencyResolution' ]; #'goal'
...
PARAM_ARGS = [ 'alias', 'expression', 'default-value' ]; #'parameter'
...
class Extractor < Mojo
  ...
  # @parameter alias="myFileName" default-value="/file.rb" expression="${someExpression}"
  # @required true
  # @parameter
  def fileName(fn)
  ...
  def execute
	...
	top_level.classes.each { |klass|
	  ...
	  unless cmnt.nil? then
		class_anno[DESC] = "<![CDATA[#{cmnt.strip}]]>"
	  end
      ...
      klass.method_list.each { |m|
        ...
        unless method_anno.empty? then
		  ...
		  unless mmnt.nil? then
			method_anno[DESC] = "<![CDATA[#{mmnt.strip}]]>"
		  end
          name = m.name
          name.chop! if name =~ %r".*?="
