    ...
    extends AbstractDebMojo
{
     ...
     * @parameter expression="usr/share/java/${project.groupId}"
     * @readonly
     * @required
     */
        ...
        throws MojoExecutionException, MojoFailureException
    {
        ...
        try
        {
            ...
            // Copy the resources
            copyFiles( outputDirectory, assemblyDirectory, null, getGroupId(), getArtifactId(), getVersion() );

            Dpkg dpkg = new Dpkg();

            // build the package
            dpkg.buildPackage( outputDirectory, assemblyDirectory, new File( getControlFileGenerator().getDebFileName() ), useFakeroot );
        }
        catch ( Exception e )
        {
    ...
    private void copyFiles( File basedir, File debian, String resourcesDir, String groupId, String artifactId,
                            String version )
        throws Exception
    {
