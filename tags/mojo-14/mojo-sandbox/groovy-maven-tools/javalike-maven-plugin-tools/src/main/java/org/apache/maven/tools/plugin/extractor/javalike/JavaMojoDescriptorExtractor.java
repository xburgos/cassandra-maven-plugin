package org.apache.maven.tools.plugin.extractor.javalike;
...
{
	// component parameter that let you choose the suffix of files to parsed
	private String srcPrefixes = ".java .groovy";
    ...
    {       
    	...
    	JavaFileVisitor javaVisitor = new JavaFileVisitor(builder);
    	PrefixesFilter prefixesFilter = new PrefixesFilter(srcPrefixes);
    	
        for ( Iterator i = project.getCompileSourceRoots().iterator(); i.hasNext(); )
        {
        	DirectoryScanner dirScan = new DirectoryScanner( new File( (String) i.next() ));
        	dirScan.addFilter(prefixesFilter);
        	dirScan.scan(javaVisitor);
        }
    ...
    private class JavaFileVisitor
    	...
    	JavaFileVisitor(JavaDocBuilder _builder) {
    		builder = _builder;
    	}
    	...
    	implements Filter
    {
    	...
    	PrefixesFilter(String srcPrefixes) {
    		prefixes = srcPrefixes.split("\\s+");
    	}
    	
    	public boolean filter(File file) {
			boolean result = false;
			for (int i = 0; i < prefixes.length; i++) {
				result |= file.getName().endsWith(prefixes[i]);
			}
			return result;
		}
    }
